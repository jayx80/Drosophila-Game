...
// Fallback color scheme used if a level in levels.js doesn't
// include its own "theme" object.
const DEFAULT_THEME = { accent: "#7CFFCB", accentDim: "#3E7A67" };
...
// Sets the game's accent colors to match the current level's
// "theme" (see levels.js). This works by changing two CSS custom
// properties on the page's root element — every part of the
// interface that's colored with var(--accent) / var(--accent-dim)
// (buttons, highlighted text, glowing neuron dots, etc) picks up
// the new colors automatically, no other code needed.
...
function submitAnswer() {
  ...
  playPhaseVideo(outcomeVideo, () => {
    if (correct) {
      // Correct answer: don't jump straight into the next level's
      // intro video. Instead let the success video's final frame
      // sit on screen, offer a "Next Level" button, and slowly
      // fade the success music down to silence underneath it.
      fadeOutMusic(2000);
      showNextLevelButton();
    } else {
...
// Shows the "Next Level" button over the frozen final frame of the
// success video, and hides the Skip button (there's nothing left
// to skip once the video has ended).
function showNextLevelButton() {
  skipVideoBtn.classList.add("hidden");
  nextLevelBtn.classList.remove("hidden");
}

// Hides the "Next Level" button and restores the Skip button for
// the next video that plays.
...
// Shared fade engine used by both fadeOutAndSwitchMusic (below)
// and fadeOutMusic. Smoothly lowers bgMusic's volume to zero over
// `durationMs`, then either switches to `newSrc` and resumes
// playback (if provided) or just leaves the music paused/silent.
function fadeMusic(newSrc, durationMs) {
  cancelMusicFade();
...
function fadeOutAndSwitchMusic(newSrc, fadeDurationMs) {
  ...
  fadeMusic(newSrc, fadeDurationMs || 600);
}

// Fades the currently-playing background music down to silence
// and leaves it stopped there — no new track follows. Used after
// a correct answer, while the "Next Level" button is on screen.
function fadeOutMusic(fadeDurationMs) {
  fadeMusic(null, fadeDurationMs || 2000);
}
